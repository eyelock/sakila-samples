...
public class RentalController {
    ...
    public ResponseEntity<String> listJson(@RequestParam(value="pageSize", required=false) String pageSize, @RequestParam(value="pageNumber", required=false) String pageNumber) {
        ...
        List<Rental> result = rentalService.findRentalEntries(pagination.getFirstResult(), pagination.getMaxResults());
        return new ResponseEntity<String>(pagination.wrapResponse(Rental.toJsonArray(result)), headers, HttpStatus.OK);
    }
    ...
    @ResponseBody
    public ResponseEntity<String> showCustomersJson(@PathVariable("customerId") Short customerId, @RequestParam(value="pageSize", required=false) String pageSize, @RequestParam(value="pageNumber", required=false) String pageNumber) {
    ...
    @ResponseBody
    public ResponseEntity<String> showFilmsJson(@PathVariable("filmId") Short filmId, @RequestParam(value="pageSize", required=false) String pageSize, @RequestParam(value="pageNumber", required=false) String pageNumber) {
    ...
    @ResponseBody
    public ResponseEntity<String> showInventoriesJson(@PathVariable("inventoryId") Short inventoryId, @RequestParam(value="pageSize", required=false) String pageSize, @RequestParam(value="pageNumber", required=false) String pageNumber) {
        Inventory inventory = inventoryService.findInventory(inventoryId.intValue());
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        if (inventory == null) {
            return new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND);
        ...
        //TODO This needs paging/sorting
        Collection<Rental> collection = rentalService.findByInventory(inventory);
