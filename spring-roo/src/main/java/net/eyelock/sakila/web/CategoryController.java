...
public class CategoryController {
    ...
    public ResponseEntity<String> listJson(@RequestParam(value="pageSize", required=false) String pageSize, @RequestParam(value="pageNumber", required=false) String pageNumber) {
        ...
        List<Category> result = categoryService.findCategoryEntries(pagination.getFirstResult(), pagination.getMaxResults());
        
        return new ResponseEntity<String>(pagination.wrapResponse(Category.toJsonArray(result)), headers, HttpStatus.OK);
    }
	
	
    @RequestMapping(value = "/{categoryId}/films", method = RequestMethod.GET, headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> showCategoriesJson(@PathVariable("categoryId") Short categoryId) {
        ...
        return new ResponseEntity<String>(pagination.wrapResponse(Film.toJsonArray(collection)), headers, HttpStatus.OK);
    }
}
