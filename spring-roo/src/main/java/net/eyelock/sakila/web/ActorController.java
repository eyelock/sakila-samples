...
public class ActorController {
    ...
    public ResponseEntity<String> listJson(@RequestParam(value="pageSize", required=false) String pageSize, @RequestParam(value="pageNumber", required=false) String pageNumber) {
        ...
        List<Actor> result = actorService.findActorEntries(pagination.getFirstResult(), pagination.getMaxResults());
        
        return new ResponseEntity<String>(pagination.wrapResponse(Actor.toJsonArray(result)), headers, HttpStatus.OK);
    }
	
	
    @RequestMapping(value = "{actorId}/films", method = RequestMethod.GET, headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> showCategoriesJson(@PathVariable("actorId") Short actorId) {
